...
def test_tweet(api):
    ...
    resp = api.post(
        '/tweet',
        data = json.dumps({'tweet' : "Hello World! test"}),
        content_type = 'application/json',
...
def test_follow(api):
    ...
    ## 먼저 사용자 1의 tweet을 확인해서 tweet 리스트가 비어 있는 것을 확인
    resp = api.get(f'/timeline/1')
    tweets = json.loads(resp.data.decode('utf-8'))
    ...
    ## 사용자 1의 tweet 확인해서 사용자 2의 tweet이 리턴되는 것을 확인
    resp = api.get(f'/timeline/1')
    tweets = json.loads(resp.data.decode('utf-8'))
...
def test_unfollow(api):
    ...
    ## 사용자 1의 tweet을 확인해서 유저 2의 tweet이 리턴되지 않는 것으로 확인
    resp = api.get(f'/timeline/1')
    tweets = json.loads(resp.data.decode('utf-8'))
